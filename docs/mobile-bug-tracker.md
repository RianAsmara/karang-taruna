...
| B-005 | 13 Sep 2026 | all bottom sheets | Nested vertical ScrollViews. The same-day keyboard fix wrapped every `BottomSheet` body in a ScrollView, but ten sheets already render their own (Pilih kegiatan, Pilih panitia, Pilih anggota, Ganti organisasi, Tetapkan peran, filters…). Two vertical scrollers nested break the inner one's gestures on Android, so those pickers would scroll unreliably or not at all. | `BottomSheet` gained a `scrollable` prop (default true); the ten sheets that bring their own ScrollView now pass `scrollable={false}`. The Pinjam barang sheet — where the original keyboard bug was reported — keeps the default, so that fix still applies. | Regression introduced and caught in the same session, before device testing. `tsc`/`expo lint` clean; not device-verified. |
