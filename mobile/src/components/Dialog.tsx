...
/** Reserved for irreversible actions only (e.g. "Keluar dari organisasi"). */
export function Dialog({ visible, title, body, confirmLabel, onConfirm, onCancel }: Props) {
        ...
        <View style={[styles.card, theme.shadow.lg]}>
          ...
          <View style={styles.buttonRow}>
            <View style={styles.buttonHalf}>
              <Button variant="secondary" label="Batal" onPress={onCancel} compact />
            </View>
            <View style={styles.buttonHalf}>
              <Button variant="primary" label={confirmLabel} onPress={onConfirm} compact />
            </View>
          </View>
        </View>
      </View>
...
function makeStyles(theme: Theme) {
  return StyleSheet.create({
    ...
    buttonRow: { flexDirection: 'row', gap: theme.space.sm },
    buttonHalf: { flex: 1 },
  });
}
